...
def globalLaplaceAverage(num, sensitivity):
    noise = laplace.rvs(0, scale=sensitivity)
    return num + noise

def localLaplaceAverage(data, sensitivity):
    result = 0
    for entry in data:
        noise = laplace.rvs(0, scale=sensitivity)
        result += (entry + noise)
...
def plotSize(data):
    ...
    plt.ylabel('Average packet sizes')
...
def accuracy_privacy_tradeoff(data):
    ...
    for i in range(1000, l, 1000):
        ...
        for eps in [0.1, 1, 3]:
            noiseResult = globalLaplaceAverage(privateResult, k/eps/1000)
...
def sample_size_factor(data):
    ...
    for i in range(1000, l, 1000):
        ...
        localNoiseResult = localLaplaceAverage(data[:i], k)
        globalNoiseResult = globalLaplaceAverage(privateResult, k/i)
